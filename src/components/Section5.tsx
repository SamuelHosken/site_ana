export default function Section5() {
  ...
  return (
    <section className="py-10 sm:py-12 md:py-16 px-4 md:px-6 bg-white">
      <div className="max-w-5xl mx-auto">
        {/* Header */}
        <div className="group mb-6 sm:mb-8 md:mb-10">
        ...
        <div className="grid lg:grid-cols-2 gap-6 sm:gap-8 md:gap-10 items-center">
          ...
          <div className="space-y-4 sm:space-y-5">
            {reasons.map((reason, index) => (
              <div
                key={index}
                className="group relative pl-5 sm:pl-6 border-l-2 border-stone-200 hover:border-primary transition-colors duration-300"
              >
                {/* Número */}
                <span className="absolute -left-2.5 sm:-left-3 top-0 w-5 h-5 sm:w-6 sm:h-6 bg-white border-2 border-stone-200 group-hover:border-primary rounded-full flex items-center justify-center text-[9px] sm:text-[10px] font-bold text-stone-400 group-hover:text-primary transition-colors duration-300">
                  {reason.number}
                </span>

                <h3 className="text-sm sm:text-base font-semibold text-gray-800 mb-1.5 sm:mb-2 group-hover:text-primary transition-colors duration-300">
                  {reason.title}
                </h3>

                {/* Linha animada */}
                <div className="h-[2px] bg-primary/20 mb-1.5 sm:mb-2 rounded-full overflow-hidden max-w-[180px]">
                  <div className="h-full w-0 bg-primary transition-all duration-500 ease-out group-hover:w-full" />
                </div>

                <p className="text-gray-600 leading-relaxed text-xs sm:text-sm">{reason.description}</p>
              </div>
            ))}
          </div>
          ...
          <div className="relative group/img">
            <div className="relative h-[220px] sm:h-[280px] md:h-[380px] bg-gradient-to-br from-stone-200 to-stone-300 rounded-lg sm:rounded-xl flex items-center justify-center overflow-hidden">
              <span className="text-stone-500 text-sm sm:text-base font-medium">[ Imagem Sala ]</span>
