...
export default function Footer() {
  ...
  return (
    <footer className="bg-gray-900 text-white">
            ...
            <ScrollReveal delay={100} className="grid grid-cols-3 md:flex gap-3 md:gap-5">
              {links.map((link, index) =>
                link.href === "#contato" ? (
                  <ContactModalTrigger
                    key={index}
                    className="text-gray-400 hover:text-primary transition-colors duration-300 text-xs text-center md:text-right cursor-pointer"
                  >
                    {link.label}
                  </ContactModalTrigger>
                ) : (
                  <a
                    key={index}
                    href={link.href}
                    className="text-gray-400 hover:text-primary transition-colors duration-300 text-xs text-center md:text-right"
                  >
                    {link.label}
                  </a>
                )
      ...
      {/* Copyright */}
      <div className="py-4 px-5 md:px-6 border-t border-gray-800">
